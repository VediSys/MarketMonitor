# -*- coding: utf-8 -*-
#!/usr/bin/env python3: Pydroid 3.9, Android 11+
""" tk_main_3i.py |410: *23122023; 11122024
    ————————————————————————————————————————————
    - *required Pydroid installs for all modules:
      ...
      ————————————————————————————————————————————
      A Pydroid yfinance portfolio monitor script
      with data, indicator and theme scripts.
      ————————————————————————————————————————————
      App script displays window of axes in figure
      with candlesticks, Bollinger Bands,
      Williams % Range (optional),
      triple Parabolic Stop And Reverse,
      dual LaGuerre and Volume,
      interpolated dual Relative Strength Index,
      interpolated dual Commodity Channel Index,
      Moving Average Convergence/Divergence.
      ————————————————————————————————————————————
      (**: for auto-didactic purposes with notes.)
_________________________________________________.
@github/vedisys ©2024 ARR
coffee $ ?: BTC: 3MZm5HF7USTC1RcNk2pTduHoZ6oBdw9pUs
            TRX: TU2PsgTHHmrmJs51vXVjrqW8LdsS8Fz1Pz
"""
# env
...
from indicator_calcs import \
    bb_calcs, laguerre_calcs,\
    psar_calcs, rsi_calcs, priceline_calcs
# theme
from themestyle import \
    Theme, Chart_Type, style_dict


FILENAME = "tk_main.py" # as in github version rename

plt.rcParams['toolbar'] = 'toolmanager'
# class def __init__ tkinter "window header default items
rpm = {
    0:'back',1:'forward',2:'help',3:'home',
    4:'pan',5:'save',6:'subplots',7:'zoom'}

# colours (0-12); matched to idx sequence!
...
af1, af2, af3, am1, am2, am3 = ( # Parabolic SAR
    0.01, 0.02, 0.03,            # default: ...0.03, ||0.05
    0.1,  0.2,  0.3)             # default: ...0.3)  ||0.5
# default: BB: 20, RSI: 14, 8, WPR: 14, dual CCI (a:14,b:31)
bb_p, ccia_p, ccib_p, rsia_p, rsib_p, wpr_p = (
    17,17,23, 5, 8,14)
#fast, slow, ema = 12,26, 9       # macd: 11,17, 5
fast, slow, ema = 11,17, 5
...
def tint_interpolations( #laga,lagb,
	...
	fb_up2, fb_dn2 = (
	    dict(y1=rsia,y2=rsib,
	        where=rsib<=rsia,color="#93c47d",alpha=0.4,interpolate=True),
	    dict(y1=rsia,y2=rsib,
	        where=rsib >rsia,color="#e06666",alpha=0.4,interpolate=True))
	fb_up2['panel'], fb_dn2['panel'] = 2, 2
	# cciA, cciB in panel 4 (now 3)
	fb_up4, fb_dn4 = (
	    dict(y1=ccia,y2=ccib,
	        where=ccib<=ccia,color=ftint[0],alpha=0.5,interpolate=True),
	    dict(y1=ccia,y2=ccib,
	        where=ccib >ccia,color=ftint[1],alpha=0.5,interpolate=True))
	fb_up4['panel'], fb_dn4['panel'] = 3, 3 # 4, 4
	# macd in panel 5 (now 3)
	fb_up5, fb_dn5 = (
	...
	fb_up5['panel'], fb_dn5['panel'] = 3, 3 # 5, 5
...
def main():
	...
	# alias' to assist logicus mods (håk for now)
	...
	# PSAR calcs
	sar1_clr,sar1_i,sar2_clr,sar2_i,sar3_clr,sar3_i = psar_calcs(
	    sar1, sar2, sar3, CURR, PREV)
	...
	rsia_panel, rsib_panel = (
	    mpf.make_addplot(rsia,color=i_clr[8],linestyle='-',panel=2,secondary_y=True),
	    mpf.make_addplot(rsib,color=i_clr[9],linestyle='-',panel=2,secondary_y=True))
	#wpr_panel = mpf.make_addplot(wpr,color=i_clr[11],linestyle='-',panel=3,secondary_y=True)
	#ccia_panel, ccib_panel = (
	#    mpf.make_addplot(ccia,color=i_clr[12],linestyle='-',panel=3,secondary_y=False),
	#    mpf.make_addplot(ccib,color=i_clr[13],linestyle='-',panel=3,secondary_y=False))
	histogram_panel, macd_panel, signal_panel = (
	    mpf.make_addplot(histogram,color=i_clr[1],panel=3,
	        type='bar',width=0.5,alpha=0.6,secondary_y=False),
	    mpf.make_addplot(macd,color=i_clr[0],panel=3,secondary_y=False),
	    mpf.make_addplot(signal,color=i_clr[2],panel=3,secondary_y=False))
	# pool axes panels for mplfinance figure
	indicators = [
	    bbu_panel, bbm_panel, bbl_panel,
	    sar1_panel,sar2_panel,sar3_panel,
	    laga_panel, lagb_panel,
	    rsia_panel, rsib_panel, # wpr_panel,
	    #ccia_panel, ccib_panel,
	    histogram_panel, signal_panel, macd_panel
	    ]
	#return # def output_to_ax_0(): ---------------------|
	c_vals, leg_fnt = {}, 19
	...
	fig, _ax = mpf.plot(_df,fill_between=interpolations,
	    style=mpf.make_mpf_style(base_mpf_style=Theme(3), y_on_right=False,
	    ...
	    xlabel=(
	        '•'+FILENAME+'•\n'+_t+' $'+str(curr_c)+'\n'+_tail),
	    panel_ratios=( 4, 1, 1, 2), volume=True,volume_panel=1, #05, #<--!
	...
	df_len, _y = len(_df), -1
	for _y in df_c:
	    _ax[0].annotate('          ' # spacing to align
	...
	# psar decision logic
	_ax[0].annotate(sar3_i,
		xy=(len(_df), curr_c),textcoords='axes fraction',
	    fontsize=leg_fnt,color=sar3_clr,xytext=(0.01, 0.78)) #,alpha=0.85)
	_ax[0].annotate(sar2_i,
		xy=(len(_df), curr_c),textcoords='axes fraction',
	    fontsize=leg_fnt,color=sar2_clr,xytext=(0.01, 0.72)) #,alpha=0.85)
	_ax[0].annotate(sar1_i,
		xy=(len(_df), curr_c),textcoords='axes fraction',
	    fontsize=leg_fnt,color=sar1_clr,xytext=(0.01, 0.66)) #,alpha=0.85)
    # laguerre decision logic type A
	_ax[0].annotate(laga_i,
	    xy=(len(_df),curr_c),textcoords='axes fraction',
	    fontsize=24,color=laga_clr,xytext=(0.04,0.04))
	# laguerre decision logic type B <--logic error #!:~+work
	...
	_ax[0].annotate(d_g,
	    xy=(len(_df),curr_c),textcoords='axes fraction',
	    fontsize=22,color=dg_clr,xytext=(0.22,0.33))
	# subpanel annotations (0-14) ---------------|
	legend_array = (
    ...
    # prep tk buttons
	button1["background"], button1["foreground"] = (
	    'lightgrey','blue')
	button1["text"] = 'repeat'
...
if __name__ == "__main__" : # tkinter dialogue window
    root = tk.Tk()
    root.configure(
        background='darkslategrey',border=0) #'#969798',border=0)
    #window.overrideredirect(True) # revert to borderless window
    root.geometry('1000x250')
    root.title('Enter ticker, range, interval')
    # spinbox selection parameters
    Ticker,Days,Interval,T_len,D_len,I_len = folio()
    spin1, spin2, spin3 = (
        tk.Spinbox(root,
            from_=0,to=T_len,
            values=Ticker,width=9,
            font=("Helvetica 15"),
            textvariable=tk.StringVar(
                value=Ticker[0]),wrap=True),
        tk.Spinbox(root,
            from_=0,to=D_len,
            values=Days,width=4,
            font=("Helvetica 15"),
            textvariable=tk.IntVar(
                value=Days[0]),wrap=True),
        tk.Spinbox(root,
            from_=0,to=I_len,
            values=Interval,width=4,
            font=("Helvetica 15"),
            textvariable=tk.StringVar(
                value=Interval[0]),wrap=True))
    # button colors, parameters and actions
    fg_clr, bg_clr= "black","lightgrey"
    button0, button1, button2, button3 = (
        tk.Button(root,
            fg=fg_clr,bg=bg_clr,font=("Consolas 5"),
            text="Click to view selection",
            command=select_0),
        tk.Button(root,
            fg=fg_clr,bg=bg_clr,relief='raised',font=("Consolas 5"),
            text="",
            command=main), # <<==<<==!!
        tk.Button(root,
            fg=fg_clr,bg=bg_clr,font=("Consolas 5"),
            ...
            command=clear_all),
        tk.Button(root,
            fg=fg_clr,bg="coral",relief='raised',font=("Consolas 5"),
            text="Exit",
            command=exit_app)) #command=exit))#window.destroy())
    # widgets placement
    ...
    # reset input parms and run dialogue GUI
    _t, _d, _i = "",0,""
    root.mainloop()
